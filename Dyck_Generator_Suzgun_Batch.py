...
class DyckLanguage():
    ...
    def lineToTensor(self, line, batch_size=1, max_len=50):
        # max_len = 50
        # tensor = torch.zeros(len(line), 1, self.n_letters)
        tensor = torch.zeros(batch_size, max_len, self.n_letters)
        # for li, letter in enumerate(line):
        #     tensor[li][0][self.letterToIndex(letter)] = 1.0
        # if len(line)<max_len:
        #     for i in range(max_len-len(line)):
        #         tensor = torch.cat((tensor, torch.tensor([[[0,0]]],dtype=torch.float32)))
        for batch in range(batch_size):
            for li, letter in enumerate(line):
                tensor[batch][li][self.letterToIndex(letter)] = 1.0
        return tensor
    def batchToTensor(self, batch, batch_size=1, max_len=50):
        tensors = torch.tensor([])
        seq_lens = [len(seq) for seq in batch]
        max_len = max(seq_lens)
        for i in range(batch_size):
            # tensors.append(self.lineToTensor(batch[i], batch_size=1, max_len=max_len))
            tensors = torch.cat((tensors, self.lineToTensor(batch[i], batch_size=1, max_len=max_len)))
        # tens = torch.tensor(tensors)
        return tensors

    def lineToTensorSigmoid(self, line, max_len=50):
    ...
    def batchToTensorSigmoid(self, batch, lengths, batch_size=1, max_len=50):
        ...
        # print('batch = ',batch)
        for i in range(batch_size):
            tensors = torch.cat((tensors, self.lineToTensorSigmoid(batch[i], max_len=max_len)))
...
# print(Dyck.batchToTensorSigmoid(['11101010','1010'],lengths=[8,4],batch_size=2,max_len=8))

# print(Dyck.lineToTensor('()'))
# tens = Dyck.lineToTensor('()')
# # print(len(tens))
#
# # print(Dyck.lineToTensorSigmoid('1110'))
#
# print(Dyck.batchToTensor(['(())', '((()))'], batch_size=2, max_len=10))
